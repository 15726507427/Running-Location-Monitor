...
public class NavUtils {
    ...
    public static double getTotalDistance(List<Point> points) {
        ...
        for (Point point : points) {
            ...
            if (count > 1 && count < points.size()) {
                totalDistance = totalDistance + getDistance(prevPoint, point);
            }
    ...
    public static Point getPosition(Point pt1, double d, double brg) {
        ...
        double lat2 = Math.asin(Math.sin(lat1) * Math.cos(d / EARTH_RADIUS_IN_METERS) + Math.cos(lat1) *
                Math.sin(d / EARTH_RADIUS_IN_METERS) * Math.cos(brgAsRadians));
    ...
    /**
     * Decode polyline to a list of points
     * */
